...
typedef enum PA_MEM_EXTENDED_PARAMETER_TYPE_E {
...
} PA_MEM_EXTENDED_PARAMETER_TYPE;

typedef struct DECLSPEC_ALIGN(8) PA_MEM_EXTENDED_PARAMETER_S {
...
} PA_MEM_EXTENDED_PARAMETER;

typedef struct PA_MEM_ADDRESS_REQUIREMENTS_S {
  PVOID  LowestStartingAddress;
  PVOID  HighestEndingAddress;
  SIZE_T Alignment;
} PA_MEM_ADDRESS_REQUIREMENTS;

#define PA_MEM_EXTENDED_PARAMETER_NONPAGED_HUGE   0x00000010

#include <winternl.h>
typedef PVOID (__stdcall *PVirtualAlloc2)(HANDLE, PVOID, SIZE_T, ULONG, ULONG, PA_MEM_EXTENDED_PARAMETER*, ULONG);
typedef LONG  (__stdcall *PNtAllocateVirtualMemoryEx)(HANDLE, PVOID*, SIZE_T*, ULONG, ULONG, PA_MEM_EXTENDED_PARAMETER*, ULONG);  // avoid NTSTATUS as it is not defined on xbox (pr #1084)
...
typedef struct PA_PROCESSOR_NUMBER_S { WORD Group; BYTE Number; BYTE Reserved; } PA_PROCESSOR_NUMBER;

typedef VOID (__stdcall *PGetCurrentProcessorNumberEx)(PA_PROCESSOR_NUMBER* ProcNumber);
typedef BOOL (__stdcall *PGetNumaProcessorNodeEx)(PA_PROCESSOR_NUMBER* Processor, PUSHORT NodeNumber);
...
static bool win_enable_large_os_pages(size_t* large_page_size)
{
  ...
  if (large_initialized) return (_pa_os_large_page_size() > 0);
  ...
  if (!ok) {
    ...
    _pa_warning_message("cannot enable large OS page support, error %lu\n", err);
  }
  return (ok!=0);
}
...
void _pa_prim_mem_init( pa_os_mem_config_t* config )
{
  ...
  if ((uintptr_t)si.lpMaximumApplicationAddress > 0) {
    const size_t vbits = PA_SIZE_BITS - pa_clz((uintptr_t)si.lpMaximumApplicationAddress);
  ...
  if (pa_option_is_enabled(pa_option_allow_large_os_pages) || pa_option_is_enabled(pa_option_reserve_huge_os_pages)) {
...
int _pa_prim_free(void* addr, size_t size ) {
  ...
  if (errcode == ERROR_INVALID_ADDRESS) {
    // In pa_os_mem_alloc_aligned the fallback path may have returned a pointer inside
    // the memory region returned by VirtualAlloc; in that case we need to free using
    // the start of the region.
    MEMORY_BASIC_INFORMATION info; _pa_memzero_var(info);
    VirtualQuery(addr, &info, sizeof(info));
    if (info.AllocationBase < addr && ((uint8_t*)addr - (uint8_t*)info.AllocationBase) < (ptrdiff_t)PA_SEGMENT_SIZE) {
      errcode = 0;
...
static void* win_virtual_alloc_prim_once(void* addr, size_t size, size_t try_alignment, DWORD flags) {
  ...
  if (addr == NULL) {
    void* hint = _pa_os_get_aligned_hint(try_alignment,size);
    if (hint != NULL) {
      void* p = VirtualAlloc(hint, size, flags, PAGE_READWRITE);
      if (p != NULL) return p;
      _pa_verbose_message("warning: unable to allocate hinted aligned OS memory (%zu bytes, error code: 0x%x, address: %p, alignment: %zu, flags: 0x%x)\n", size, GetLastError(), hint, try_alignment, flags);
      // fall through on error
    }
  }
  #endif
  // on modern Windows try use VirtualAlloc2 for aligned allocation
  if (addr == NULL && try_alignment > 1 && (try_alignment % _pa_os_page_size()) == 0 && pVirtualAlloc2 != NULL) {
    PA_MEM_ADDRESS_REQUIREMENTS reqs = { 0, 0, 0 };
    reqs.Alignment = try_alignment;
    PA_MEM_EXTENDED_PARAMETER param = { {0, 0}, {0} };
    param.Type.Type = MiMemExtendedParameterAddressRequirements;
    param.Arg.Pointer = &reqs;
    void* p = (*pVirtualAlloc2)(GetCurrentProcess(), addr, size, flags, PAGE_READWRITE, &param, 1);
    if (p != NULL) return p;
    _pa_warning_message("unable to allocate aligned OS memory (0x%zx bytes, error code: 0x%x, address: %p, alignment: 0x%zx, flags: 0x%x)\n", size, GetLastError(), addr, try_alignment, flags);
    // fall through on error
  }
  // last resort
...
static void* win_virtual_alloc_prim(void* addr, size_t size, size_t try_alignment, DWORD flags) {
  long max_retry_msecs = pa_option_get_clamp(pa_option_retry_on_oom, 0, 2000);  // at most 2 seconds
  ...
  for (long tries = 1; tries <= 10; tries++) {          // try at most 10 times (=2200ms)
    ...
    if (p != NULL) {
    ...
    else if (max_retry_msecs > 0 && (try_alignment <= 2*PA_SEGMENT_ALIGN) &&
      ...
      _pa_warning_message("out-of-memory on OS allocation, try again... (attempt %lu, 0x%zx bytes, error code: 0x%x, address: %p, alignment: 0x%zx, flags: 0x%x)\n", tries, size, GetLastError(), addr, try_alignment, flags);
...
static void* win_virtual_alloc(void* addr, size_t size, size_t try_alignment, DWORD flags, bool large_only, bool allow_large, bool* is_large) {
  pa_assert_internal(!(large_only && !allow_large));
  static _Atomic(size_t) large_page_try_ok; // = 0;
  void* p = NULL;
  // Try to allocate large OS pages (2MiB) if allowed or required.
  if ((large_only || (_pa_os_canuse_large_page(size, try_alignment) && pa_option_is_enabled(pa_option_allow_large_os_pages)))
      && allow_large && (flags&MEM_COMMIT)!=0 && (flags&MEM_RESERVE)!=0)
  {
    size_t try_ok = pa_atomic_load_acquire(&large_page_try_ok);
    if (!large_only && try_ok > 0) {
      // if a large page allocation fails, it seems the calls to VirtualAlloc get very expensive.
      // therefore, once a large page allocation failed, we don't try again for `large_page_try_ok` times.
      pa_atomic_cas_strong_acq_rel(&large_page_try_ok, &try_ok, try_ok - 1);
    }
    else {
      ...
      *is_large = true;
      ...
      if (p == NULL) {
        pa_atomic_store_release(&large_page_try_ok,10UL);  // on error, don't try again for the next N allocations
      }
    }
  }
  ...
  //if (p == NULL) { _pa_warning_message("unable to allocate OS memory (%zu bytes, error code: 0x%x, address: %p, alignment: %zu, flags: 0x%x, large only: %d, allow large: %d)\n", size, GetLastError(), addr, try_alignment, flags, large_only, allow_large); }
  return p;
}
...
int _pa_prim_alloc(void* hint_addr, size_t size, size_t try_alignment, bool commit, bool allow_large, bool* is_large, bool* is_zero, void** addr) {
  ...
  pa_assert_internal(commit || !allow_large);
  pa_assert_internal(try_alignment > 0);
  *is_zero = true;
...
int _pa_prim_commit(void* addr, size_t size, bool* is_zero) {
  *is_zero = false;
  ...
  _MEMORY_BASIC_INFORMATION meminfo; _pa_memzero_var(meminfo);
...
static void* _pa_prim_alloc_huge_os_pagesx(void* hint_addr, size_t size, int numa_node)
{
  const DWORD flags = MEM_LARGE_PAGES | MEM_COMMIT | MEM_RESERVE;

  win_enable_large_os_pages(NULL);

  PA_MEM_EXTENDED_PARAMETER params[3] = { {{0,0},{0}},{{0,0},{0}},{{0,0},{0}} };
  // on modern Windows try use NtAllocateVirtualMemoryEx for 1GiB huge pages
  static bool pa_huge_pages_available = true;
  if (pNtAllocateVirtualMemoryEx != NULL && pa_huge_pages_available) {
    params[0].Type.Type = MiMemExtendedParameterAttributeFlags;
    params[0].Arg.ULong64 = PA_MEM_EXTENDED_PARAMETER_NONPAGED_HUGE;
    ULONG param_count = 1;
    if (numa_node >= 0) {
      param_count++;
    ...
    if (err == 0 && base != NULL) {
      ...
      pa_huge_pages_available = false; // don't try further huge pages
      _pa_warning_message("unable to allocate using huge (1GiB) pages, trying large (2MiB) pages instead (status 0x%lx)\n", err);
    }
  }
...
int _pa_prim_alloc_huge_os_pages(void* hint_addr, size_t size, int numa_node, bool* is_zero, void** addr) {
  *is_zero = true;
  *addr = _pa_prim_alloc_huge_os_pagesx(hint_addr,size,numa_node);
  return (*addr != NULL ? 0 : (int)GetLastError());
}
...
static pa_msecs_t pa_to_msecs(LARGE_INTEGER t) {
  ...
  return (pa_msecs_t)(t.QuadPart / mfreq.QuadPart);
}

pa_msecs_t _pa_prim_clock_now(void) {
  LARGE_INTEGER t;
  QueryPerformanceCounter(&t);
  return pa_to_msecs(t);
}
...
static pa_msecs_t filetime_msecs(const FILETIME* ftime) {
  ULARGE_INTEGER i;
  i.LowPart = ftime->dwLowDateTime;
  i.HighPart = ftime->dwHighDateTime;
  pa_msecs_t msecs = (i.QuadPart / 10000); // FILETIME is in 100 nano seconds
  return msecs;
}
...
void _pa_prim_process_info(pa_process_info_t* pinfo)
{
  ...
  // get process info
  PROCESS_MEMORY_COUNTERS info; _pa_memzero_var(info);
  if (pGetProcessMemoryInfo != NULL) {
...
void _pa_prim_out_stderr( const char* msg )
{
  ...
  if (!_pa_preloading()) {
    ...
    if (hcon == INVALID_HANDLE_VALUE) {
      ...
      #ifdef PA_HAS_CONSOLE_IO
      CONSOLE_SCREEN_BUFFER_INFO sbi;
      hconIsConsole = ((hcon != INVALID_HANDLE_VALUE) && GetConsoleScreenBufferInfo(hcon, &sbi));
      #endif
    }
    const size_t len = _pa_strlen(msg);
...
#if defined(PA_USE_RTLGENRANDOM) // || defined(__cplusplus)
...
pa_decl_externc BOOLEAN NTAPI RtlGenRandom(PVOID RandomBuffer, ULONG RandomBufferLength);
...
#if PA_WIN_USE_FIXED_TLS==1
pa_decl_cache_align size_t _pa_win_tls_offset = 0;
#endif

//static void pa_debug_out(const char* s) {
//  HANDLE h = GetStdHandle(STD_ERROR_HANDLE);
//  WriteConsole(h, s, (DWORD)_pa_strlen(s), NULL, NULL);
//}

static void pa_win_tls_init(DWORD reason) {
  if (reason==DLL_PROCESS_ATTACH || reason==DLL_THREAD_ATTACH) {
    #if PA_WIN_USE_FIXED_TLS==1  // we must allocate a TLS slot dynamically
    if (_pa_win_tls_offset == 0 && reason == DLL_PROCESS_ATTACH) {
      const DWORD tls_slot = TlsAlloc();  // usually returns slot 1
      if (tls_slot == TLS_OUT_OF_INDEXES) {
        _pa_error_message(EFAULT, "unable to allocate the a TLS slot (rebuild without PA_WIN_USE_FIXED_TLS?)\n");
      }
      _pa_win_tls_offset = (size_t)tls_slot * sizeof(void*);
    }
    #endif
    #if PA_HAS_TLS_SLOT >= 2  // we must initialize the TLS slot before any allocation
    if (pa_prim_get_default_heap() == NULL) {
      _pa_heap_set_default_direct((pa_heap_t*)&_pa_heap_empty);
      #if PA_DEBUG && PA_WIN_USE_FIXED_TLS==1
      void* const p = TlsGetValue((DWORD)(_pa_win_tls_offset / sizeof(void*)));
      pa_assert_internal(p == (void*)&_pa_heap_empty);
      #endif
    }
    #endif
  }
...
static void NTAPI pa_win_main(PVOID module, DWORD reason, LPVOID reserved) {
  PA_UNUSED(reserved);
  PA_UNUSED(module);
  pa_win_tls_init(reason);
  ...
  else if (reason==DLL_THREAD_DETACH && !_pa_is_redirected()) {
  ...
  #define PA_PRIM_HAS_PROCESS_ATTACH  1

  // Windows DLL: easy to hook into process_init and thread_done
  BOOL WINAPI DllMain(HINSTANCE inst, DWORD reason, LPVOID reserved) {
    pa_win_main((PVOID)inst,reason,reserved);
    return TRUE;
  }

  // nothing to do since `_pa_thread_done` is handled through the DLL_THREAD_DETACH event.
...
#elif !defined(PA_WIN_USE_FLS)
  #define PA_PRIM_HAS_PROCESS_ATTACH  1

  static void NTAPI pa_win_main_attach(PVOID module, DWORD reason, LPVOID reserved) {
    if (reason == DLL_PROCESS_ATTACH || reason == DLL_THREAD_ATTACH) {
      pa_win_main(module, reason, reserved);
    }
  }
  static void NTAPI pa_win_main_detach(PVOID module, DWORD reason, LPVOID reserved) {
  ...
  extern "C" {
    ...
    #pragma comment(linker, "/INCLUDE:_pa_tls_callback_pre")
    #pragma comment(linker, "/INCLUDE:_pa_tls_callback_post")
    #pragma const_seg(".CRT$XLB")
    extern const PIMAGE_TLS_CALLBACK _pa_tls_callback_pre[];
    const PIMAGE_TLS_CALLBACK _pa_tls_callback_pre[] = { &pa_win_main_attach };
    #pragma const_seg()
    #pragma const_seg(".CRT$XLY")
    extern const PIMAGE_TLS_CALLBACK _pa_tls_callback_post[];
    const PIMAGE_TLS_CALLBACK _pa_tls_callback_post[] = { &pa_win_main_detach };
    #pragma const_seg()
  #else
    #pragma comment(linker, "/INCLUDE:__tls_used")
    #pragma comment(linker, "/INCLUDE:__pa_tls_callback_pre")
    #pragma comment(linker, "/INCLUDE:__pa_tls_callback_post")
    #pragma data_seg(".CRT$XLB")
    PIMAGE_TLS_CALLBACK _pa_tls_callback_pre[] = { &pa_win_main_attach };
    #pragma data_seg()
    #pragma data_seg(".CRT$XLY")
    PIMAGE_TLS_CALLBACK _pa_tls_callback_post[] = { &pa_win_main_detach };
    #pragma data_seg()
  #endif
  ...
  void _pa_prim_thread_associate_default_heap(pa_heap_t* heap) {
    ...
    static int pa_process_attach(void) {
      pa_win_main(NULL,DLL_PROCESS_ATTACH,NULL);
      atexit(&_pa_auto_process_done);
      return 0;
    }
    typedef int(*pa_crt_callback_t)(void);
    #if defined(_WIN64)
      #pragma comment(linker, "/INCLUDE:_pa_tls_callback")
      #pragma section(".CRT$XIU", long, read)
    #else
      #pragma comment(linker, "/INCLUDE:__pa_tls_callback")
    #endif
    #pragma data_seg(".CRT$XIU")
    pa_decl_externc pa_crt_callback_t _pa_tls_callback[] = { &pa_process_attach };
    #pragma data_seg()
  #endif

  // use the fiber api for calling `_pa_thread_done`.
  #include <fibersapi.h>
  ...
  static DWORD pa_fls_key = (DWORD)(-1);

  static void NTAPI pa_fls_done(PVOID value) {
    pa_heap_t* heap = (pa_heap_t*)value;
    if (heap != NULL) {
      _pa_thread_done(heap);
      FlsSetValue(pa_fls_key, NULL);  // prevent recursion as _pa_thread_done may set it back to the main heap, issue #672
    }
  }

  void _pa_prim_thread_init_auto_done(void) {
    pa_fls_key = FlsAlloc(&pa_fls_done);
  }
  ...
  void _pa_prim_thread_associate_default_heap(pa_heap_t* heap) {
    pa_assert_internal(pa_fls_key != (DWORD)(-1));
    FlsSetValue(pa_fls_key, heap);
  }
#endif
...
// ----------------------------------------------------
#if defined(PA_SHARED_LIB) && !defined(PA_WIN_NOREDIRECT)
  #define PA_PRIM_HAS_ALLOCATOR_INIT 1

  static bool pa_redirected = false;   // true if malloc redirects to pa_malloc

  bool _pa_is_redirected(void) {
    return pa_redirected;
  }

  #ifdef __cplusplus
  extern "C" {
  #endif
  pa_decl_export void _pa_redirect_entry(DWORD reason) {
  ...
  __declspec(dllimport) bool pa_cdecl pa_allocator_init(const char** message);
  __declspec(dllimport) void pa_cdecl pa_allocator_done(void);
  #ifdef __cplusplus
  }
  #endif
  bool _pa_allocator_init(const char** message) {
    return pa_allocator_init(message);
  }
