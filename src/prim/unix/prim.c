...
#define PA_UNIX_LARGE_PAGE_SIZE (2*PA_MiB) // TODO: can we query the OS for this?
...
#if defined(PA_HAS_SYSCALL_H) && defined(SYS_open) && defined(SYS_close) && defined(SYS_read) && defined(SYS_access)

static inline int pa_prim_open(const char* fpath, int open_flags) {
  return syscall(SYS_open,fpath,open_flags,0);
}
static inline ssize_t pa_prim_read(int fd, void* buf, size_t bufsize) {
  return syscall(SYS_read,fd,buf,bufsize);
}
static inline int pa_prim_close(int fd) {
  return syscall(SYS_close,fd);
}
static inline int pa_prim_access(const char *fpath, int mode) {
...
static bool unix_detect_overcommit(void) {
  ...
  #if defined(__linux__)
    int fd = pa_prim_open("/proc/sys/vm/overcommit_memory", O_RDONLY);
    if (fd >= 0) {
      char buf[32];
      ssize_t nread = pa_prim_read(fd, &buf, sizeof(buf));
      pa_prim_close(fd);
...
static void unix_detect_physical_memory( size_t page_size, size_t* physical_memory_in_kib ) {
    ...
    if (err==0 && physical_memory > 0) {
      const int64_t phys_in_kib = physical_memory / PA_KiB;
    ...
    PA_UNUSED(page_size);
    struct sysinfo info; _pa_memzero_var(info);
    const int err = sysinfo(&info);
    if (err==0 && info.totalram > 0 && info.totalram <= SIZE_MAX) {
      *physical_memory_in_kib = (size_t)info.totalram / PA_KiB;
    }
    ...
    const size_t psize_in_kib = page_size / PA_KiB;
...
void _pa_prim_mem_init( pa_os_mem_config_t* config )
{
  ...
  config->large_page_size = PA_UNIX_LARGE_PAGE_SIZE;
  ...
  #if defined(PA_NO_THP)
  if (true)
  #else
  if (!pa_option_is_enabled(pa_option_allow_thp)) // disable THP if requested through an option
  #endif
  {
    int val = 0;
...
static void* unix_mmap_prim_aligned(void* addr, size_t size, size_t try_alignment, int protect_flags, int flags, int fd) {
  PA_UNUSED(try_alignment);
  void* p = NULL;
  #if defined(MAP_ALIGNED)  // BSD
  if (addr == NULL && try_alignment > 1 && (try_alignment % _pa_os_page_size()) == 0) {
    size_t n = pa_bsr(try_alignment);
    if (((size_t)1 << n) == try_alignment && n >= 12 && n <= 30) {  // alignment is a power of 2 and 4096 <= alignment <= 1GiB
      p = unix_mmap_prim(addr, size, protect_flags, flags | MAP_ALIGNED(n), fd);
      if (p==MAP_FAILED || !_pa_is_aligned(p,try_alignment)) {
        int err = errno;
        _pa_trace_message("unable to directly request aligned OS memory (error: %d (0x%x), size: 0x%zx bytes, alignment: 0x%zx, hint address: %p)\n", err, err, size, try_alignment, addr);
      }
  ...
  #endif
  #if (PA_INTPTR_SIZE >= 8) && !defined(MAP_ALIGNED)
  ...
  if (addr == NULL) {
    ...
    if (hint != NULL) {
      ...
      if (p==MAP_FAILED || !_pa_is_aligned(p,try_alignment)) {
        #if PA_TRACK_ENABLED  // asan sometimes does not instrument errno correctly?
        int err = 0;
        #else
        int err = errno;
        #endif
        _pa_trace_message("unable to directly request hinted aligned OS memory (error: %d (0x%x), size: 0x%zx bytes, alignment: 0x%zx, hint address: %p)\n", err, err, size, try_alignment, hint);
      }
      if (p!=MAP_FAILED) return p;
      // fall back to regular mmap
    }
...
static int unix_mmap_fd(void) {
  ...
  int os_tag = (int)pa_option_get(pa_option_os_tag);
...
static void* unix_mmap(void* addr, size_t size, size_t try_alignment, int protect_flags, bool large_only, bool allow_large, bool* is_large) {
  ...
  if (allow_large && (large_only || (_pa_os_canuse_large_page(size, try_alignment) && pa_option_is_enabled(pa_option_allow_large_os_pages)))) {
    ...
    if (!large_only && try_ok > 0) {
      ...
      if (large_only && (size % PA_GiB) == 0 && pa_huge_pages_available) {
        lflags |= MAP_HUGE_1GB;
      }
      else
      #endif
      ...
      if (large_only || lflags != flags) {
        ...
        *is_large = true;
        ...
        if (p == NULL && (lflags & MAP_HUGE_1GB) == MAP_HUGE_1GB) {
          pa_huge_pages_available = false; // don't try huge 1GiB pages again
          if (large_only) {
            _pa_warning_message("unable to allocate huge (1GiB) page, trying large (2MiB) pages instead (errno: %i)\n", errno);
          }
        ...
        if (p == NULL) {
          pa_atomic_store_release(&large_page_try_ok, (size_t)8);  // on error, don't try again for the next N allocations
        }
      }
    }
  }
  ...
  if (p == NULL) {
    *is_large = false;
    ...
    #if !defined(PA_NO_THP)
    if (p != NULL && allow_large && pa_option_is_enabled(pa_option_allow_thp) && _pa_os_canuse_large_page(size, try_alignment)) {
      ...
      cmd.mha_pagesize = _pa_os_large_page_size();
...
int _pa_prim_alloc(void* hint_addr, size_t size, size_t try_alignment, bool commit, bool allow_large, bool* is_large, bool* is_zero, void** addr) {
  ...
  if (hint_addr == NULL && size >= 8*PA_UNIX_LARGE_PAGE_SIZE && try_alignment > 1 && _pa_is_power_of_two(try_alignment) && try_alignment < PA_UNIX_LARGE_PAGE_SIZE) {
    try_alignment = PA_UNIX_LARGE_PAGE_SIZE; // try to align along large page size for larger allocations
  }
...
static void unix_mprotect_hint(int err) {
  #if defined(__linux__) && (PA_SECURE>=2) // guard page around every palloc page
  if (err == ENOMEM) {
    _pa_warning_message("The next warning may be caused by a low memory map limit.\n"
...
int _pa_prim_commit(void* start, size_t size, bool* is_zero) {
...
int _pa_prim_reuse(void* start, size_t size) {
  PA_UNUSED(start); PA_UNUSED(size);
...
int _pa_prim_decommit(void* start, size_t size, bool* needs_recommit) {
  int err = 0;
...
int _pa_prim_reset(void* start, size_t size) {
  int err = 0;
  ...
  static _Atomic(size_t) advice = PA_ATOMIC_VAR_INIT(MADV_FREE);
  int oadvice = (int)pa_atomic_load_relaxed(&advice);
  while ((err = unix_madvise(start, size, oadvice)) != 0 && errno == EAGAIN) { errno = 0;  };
  if (err != 0 && errno == EINVAL && oadvice == MADV_FREE) {
    // if MADV_FREE is not supported, fall back to MADV_DONTNEED from now on
    pa_atomic_store_release(&advice, (size_t)MADV_DONTNEED);
    err = unix_madvise(start, size, MADV_DONTNEED);
  }
  #else
  err = unix_madvise(start, size, MADV_DONTNEED);
...
int _pa_prim_protect(void* start, size_t size, bool protect) {
...
#if (PA_INTPTR_SIZE >= 8) && !defined(__HAIKU__) && !defined(__CYGWIN__)

#ifndef MPOL_PREFERRED
#define MPOL_PREFERRED 1
#endif

#if defined(PA_HAS_SYSCALL_H) && defined(SYS_mbind)
static long pa_prim_mbind(void* start, unsigned long len, unsigned long mode, const unsigned long* nmask, unsigned long maxnode, unsigned flags) {
  return syscall(SYS_mbind, start, len, mode, nmask, maxnode, flags);
}
#else
static long pa_prim_mbind(void* start, unsigned long len, unsigned long mode, const unsigned long* nmask, unsigned long maxnode, unsigned flags) {
  PA_UNUSED(start); PA_UNUSED(len); PA_UNUSED(mode); PA_UNUSED(nmask); PA_UNUSED(maxnode); PA_UNUSED(flags);
  return 0;
}
#endif
...
int _pa_prim_alloc_huge_os_pages(void* hint_addr, size_t size, int numa_node, bool* is_zero, void** addr) {
  bool is_large = true;
  *is_zero = true;
  *addr = unix_mmap(hint_addr, size, PA_SEGMENT_SIZE, PROT_READ | PROT_WRITE, true, true, &is_large);
  if (*addr != NULL && numa_node >= 0 && numa_node < 8*PA_INTPTR_SIZE) { // at most 64 nodes
    unsigned long numa_mask = (1UL << numa_node);
    // TODO: does `mbind` work correctly for huge OS pages? should we
    // use `set_mempolicy` before calling mmap instead?
    // see: <https://lkml.org/lkml/2017/2/9/875>
    long err = pa_prim_mbind(*addr, size, MPOL_PREFERRED, &numa_mask, 8*PA_INTPTR_SIZE, 0);
    if (err != 0) {
      err = errno;
      _pa_warning_message("failed to bind huge (1GiB) pages to numa node %d (error: %d (0x%x))\n", numa_node, err, err);
    }
  }
  return (*addr != NULL ? 0 : errno);
}
...
size_t _pa_prim_numa_node(void) {
  #if defined(PA_HAS_SYSCALL_H) && defined(SYS_getcpu)
...
size_t _pa_prim_numa_node_count(void) {
  ...
  for(node = 0; node < 256; node++) {
    ...
    _pa_snprintf(buf, 127, "/sys/devices/system/node/node%u", node + 1);
    if (pa_prim_access(buf,R_OK) != 0) break;
  }
  return (node+1);
}
...
static pa_msecs_t timeval_secs(const struct timeval* tv) {
  return ((pa_msecs_t)tv->tv_sec * 1000L) + ((pa_msecs_t)tv->tv_usec / 1000L);
}
...
bool _pa_prim_getenv(const char* name, char* result, size_t result_size) {
  ...
  const size_t len = _pa_strlen(name);
  if (len == 0) return false;
  char** env = pa_get_environ();
  if (env == NULL) return false;
  // compare up to 10000 entries
  for (int i = 0; i < 10000 && env[i] != NULL; i++) {
    const char* s = env[i];
    if (_pa_strnicmp(name, s, len) == 0 && s[len] == '=') { // case insensitive
      // found it
      _pa_strlcpy(result, s + len + 1, result_size);
      return true;
    }
  }
  return false;
...
bool _pa_prim_random_buf(void* buf, size_t buf_len) {
  ...
  #if defined(PA_HAS_SYSCALL_H) && defined(SYS_getrandom)
    #ifndef GRND_NONBLOCK
    #define GRND_NONBLOCK (1)
    #endif
    static _Atomic(uintptr_t) no_getrandom; // = 0
    if (pa_atomic_load_acquire(&no_getrandom)==0) {
      ssize_t ret = syscall(SYS_getrandom, buf, buf_len, GRND_NONBLOCK);
      if (ret >= 0) return (buf_len == (size_t)ret);
      if (errno != ENOSYS) return false;
      pa_atomic_store_release(&no_getrandom, (uintptr_t)1); // don't call again, and fall back to /dev/urandom
    }
  #endif
  int flags = O_RDONLY;
  ...
  flags |= O_CLOEXEC;
  #endif
  int fd = pa_prim_open("/dev/urandom", flags);
  if (fd < 0) return false;
  size_t count = 0;
  while(count < buf_len) {
    ssize_t ret = pa_prim_read(fd, (char*)buf + count, buf_len - count);
    if (ret<=0) {
...
void _pa_prim_thread_done_auto_done(void) {
  if (_pa_heap_default_key != (pthread_key_t)(-1)) {  // do not leak the key, see issue #809
    pthread_key_delete(_pa_heap_default_key);
  }
}
