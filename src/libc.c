...
#include "palloc/prim.h"      // pa_prim_getenv
...
int _pa_strnicmp(const char* s, const char* t, size_t n) {
  ...
  for (; *s != 0 && *t != 0 && n > 0; s++, t++, n--) {
    if (_pa_toupper(*s) != _pa_toupper(*t)) break;
  }
  return (n == 0 ? 0 : *s - *t);
}
...
void _pa_strlcat(char* dest, const char* src, size_t dest_size) {
  ...
  // and catenate
  _pa_strlcpy(dest, src, dest_size);
}
...
bool _pa_getenv(const char* name, char* result, size_t result_size) {
  ...
  return _pa_prim_getenv(name,result,result_size);
}
#endif

// --------------------------------------------------------
// Define our own limited `_pa_vsnprintf` and `_pa_snprintf`
...
static void pa_outc(char c, char** out, char* end) {
  char* p = *out;
  if (p >= end) return;
  *p = c;
  *out = p + 1;
...
static void pa_outs(const char* s, char** out, char* end) {
  if (s == NULL) return;
  char* p = *out;
  while (*s != 0 && p < end) {
    *p++ = *s++;
...
static void pa_out_fill(char fill, size_t len, char** out, char* end) {
  char* p = *out;
...
static void pa_out_alignright(char fill, char* start, size_t len, size_t extra, char* end) {
...
static void pa_out_num(uintmax_t x, size_t base, char prefix, char** out, char* end)
{
  if (x == 0 || base == 0 || base > 16) {
    if (prefix != 0) { pa_outc(prefix, out, end); }
    pa_outc('0',out,end);
  }
  else {
    ...
    while (x > 0) {
      ...
      pa_outc((digit <= 9 ? '0' + digit : 'A' + digit - 10),out,end);
      x = x / base;
    }
    if (prefix != 0) {
      pa_outc(prefix, out, end);
    }
...
#define PA_NEXTC()  c = *in; if (c==0) break; in++;
...
int _pa_vsnprintf(char* buf, size_t bufsize, const char* fmt, va_list args) {
  ...
  while (true) {
    ...
    if (c != '%') {
      ...
      if (c == '+' || c == ' ') { numplus = c; PA_NEXTC(); }
      if (c == '-') { alignright = false; PA_NEXTC(); }
      if (c == '0') { fill = '0'; PA_NEXTC(); }
      if (c >= '1' && c <= '9') {
        width = (c - '0'); PA_NEXTC();
        while (c >= '0' && c <= '9') {
          width = (10 * width) + (c - '0'); PA_NEXTC();
        }
        if (c == 0) break;  // extra check due to while
      }
      if (c == 'z' || c == 't' || c == 'L') { numtype = c; PA_NEXTC(); }
      else if (c == 'l') {
        numtype = c; PA_NEXTC();
        if (c == 'l') { numtype = 'L'; PA_NEXTC(); }
      }
      ...
      if (c == 's') {
        ...
        pa_out_num(x, (c == 'x' || c == 'p' ? 16 : 10), numplus, &out, end);
      }
      else if (c == 'i' || c == 'd') {
        // signed
        intmax_t x = 0;
        ...
        pa_out_num((uintmax_t)x, 10, pre, &out, end);
      }
      else if (c >= ' ' && c <= '~') {
        // unknown format
        pa_outc('%', &out, end);
      ...
      // fill & align
      pa_assert_internal(out <= end);
      pa_assert_internal(out >= start);
      const size_t len = out - start;
      if (len < width) {
        pa_out_fill(fill, width - len, &out, end);
        if (alignright && out <= end) {
          pa_out_alignright(fill, start, len, width - len, end);
        }
      }
    }
  }
...
int _pa_snprintf(char* buf, size_t buflen, const char* fmt, ...) {
  va_list args;
  va_start(args, fmt);
  const int written = _pa_vsnprintf(buf, buflen, fmt, args);
  va_end(args);
  return written;
}
...
#if PA_SIZE_SIZE == 4
#define pa_mask_even_bits32      (0x55555555)
#define pa_mask_even_pairs32     (0x33333333)
#define pa_mask_even_nibbles32   (0x0F0F0F0F)

// sum of all the bytes in `x` if it is guaranteed that the sum < 256!
static size_t pa_byte_sum32(uint32_t x) {
...
static size_t pa_popcount_generic32(uint32_t x) {
  ...
  x = x - ((x >> 1) & pa_mask_even_bits32);
  // add the 2-bit pair results
  x = (x & pa_mask_even_pairs32) + ((x >> 2) & pa_mask_even_pairs32);
  // add the 4-bit nibble results
  x = (x + (x >> 4)) & pa_mask_even_nibbles32;
  // each byte now has a count of its bits, we can sum them now:
  return pa_byte_sum32(x);
}

pa_decl_noinline size_t _pa_popcount_generic(size_t x) {
  return pa_popcount_generic32(x);
}

#else
#define pa_mask_even_bits64      (0x5555555555555555)
#define pa_mask_even_pairs64     (0x3333333333333333)
#define pa_mask_even_nibbles64   (0x0F0F0F0F0F0F0F0F)

// sum of all the bytes in `x` if it is guaranteed that the sum < 256!
static size_t pa_byte_sum64(uint64_t x) {
  x += (x << 8);
  x += (x << 16);
  x += (x << 32);
  return (size_t)(x >> 56);
...
static size_t pa_popcount_generic64(uint64_t x) {
  x = x - ((x >> 1) & pa_mask_even_bits64);
  x = (x & pa_mask_even_pairs64) + ((x >> 2) & pa_mask_even_pairs64);
  x = (x + (x >> 4)) & pa_mask_even_nibbles64;
  return pa_byte_sum64(x);
}

pa_decl_noinline size_t _pa_popcount_generic(size_t x) {
  return pa_popcount_generic64(x);
}
#endif
