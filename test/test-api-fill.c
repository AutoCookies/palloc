...
int main(void) {
  ...
  CHECK_BODY("zeroinit-zalloc-small") {
    ...
    uint8_t* p = (uint8_t*)pa_zalloc(zalloc_size);
  ...
  CHECK_BODY("zeroinit-zalloc_small") {
    size_t zalloc_size = PA_SMALL_SIZE_MAX / 2;
    uint8_t* p = (uint8_t*)pa_zalloc_small(zalloc_size);
    result = check_zero_init(p, zalloc_size);
    pa_free(p);
  };

  CHECK_BODY("zeroinit-calloc-small") {
    size_t calloc_size = PA_SMALL_SIZE_MAX / 2;
    uint8_t* p = (uint8_t*)pa_calloc(calloc_size, 1);
  ...
  CHECK_BODY("zeroinit-rezalloc-small") {
    ...
    result = check_zero_init(p, zalloc_size);
    zalloc_size *= 3;
    p = (uint8_t*)pa_rezalloc(p, zalloc_size);
    result &= check_zero_init(p, zalloc_size);
    pa_free(p);
  };
  ...
  CHECK_BODY("zeroinit-recalloc-small") {
    ...
    result = check_zero_init(p, calloc_size);
    calloc_size *= 3;
    p = (uint8_t*)pa_recalloc(p, calloc_size, 1);
    result &= check_zero_init(p, calloc_size);
    pa_free(p);
  };
  ...
  CHECK_BODY("zeroinit-calloc_aligned-small") {
    size_t calloc_size = PA_SMALL_SIZE_MAX / 2;
    uint8_t* p = (uint8_t*)pa_calloc_aligned(calloc_size, 1, PA_MAX_ALIGN_SIZE * 2);
    result = check_zero_init(p, calloc_size);
    pa_free(p);
  };
  ...
  CHECK_BODY("zeroinit-recalloc_aligned-small") {
    ...
    result = check_zero_init(p, calloc_size);
    calloc_size *= 3;
    p = (uint8_t*)pa_recalloc_aligned(p, calloc_size, 1, PA_MAX_ALIGN_SIZE * 2);
    result &= check_zero_init(p, calloc_size);
    pa_free(p);
  };
...
#if (PA_DEBUG >= 2) && !PA_TSAN
  // ---------------------------------------------------
  // Debug filling
  // ---------------------------------------------------
  CHECK_BODY("uninit-malloc-small") {
    size_t malloc_size = PA_SMALL_SIZE_MAX / 2;
    uint8_t* p = (uint8_t*)pa_malloc(malloc_size);
  ...
  CHECK_BODY("uninit-malloc_small") {
    size_t malloc_size = PA_SMALL_SIZE_MAX / 2;
    uint8_t* p = (uint8_t*)pa_malloc_small(malloc_size);
    result = check_debug_fill_uninit(p, malloc_size);
    pa_free(p);
  };
  ...
  CHECK_BODY("uninit-realloc-small") {
    ...
    result = check_debug_fill_uninit(p, malloc_size);
    malloc_size *= 3;
    p = (uint8_t*)pa_realloc(p, malloc_size);
    result &= check_debug_fill_uninit(p, malloc_size);
    pa_free(p);
  };
  ...
  CHECK_BODY("uninit-mallocn-small") {
    size_t malloc_size = PA_SMALL_SIZE_MAX / 2;
    uint8_t* p = (uint8_t*)pa_mallocn(malloc_size, 1);
    result = check_debug_fill_uninit(p, malloc_size);
    pa_free(p);
  };
  ...
  CHECK_BODY("uninit-reallocn-small") {
    ...
    result = check_debug_fill_uninit(p, malloc_size);
    malloc_size *= 3;
    p = (uint8_t*)pa_reallocn(p, malloc_size, 1);
    result &= check_debug_fill_uninit(p, malloc_size);
    pa_free(p);
  };
  ...
  CHECK_BODY("uninit-malloc_aligned-small") {
    size_t malloc_size = PA_SMALL_SIZE_MAX / 2;
    uint8_t* p = (uint8_t*)pa_malloc_aligned(malloc_size, PA_MAX_ALIGN_SIZE * 2);
    result = check_debug_fill_uninit(p, malloc_size);
    pa_free(p);
  };
  ...
  CHECK_BODY("uninit-realloc_aligned-small") {
    ...
    result = check_debug_fill_uninit(p, malloc_size);
    malloc_size *= 3;
    p = (uint8_t*)pa_realloc_aligned(p, malloc_size, PA_MAX_ALIGN_SIZE * 2);
    result &= check_debug_fill_uninit(p, malloc_size);
    pa_free(p);
  };
  ...
  #if !(PA_TRACK_VALGRIND || PA_TRACK_ASAN || PA_GUARDED)
...
#if PA_DEBUG >= 2
bool check_debug_fill_uninit(uint8_t* p, size_t size) {
#if PA_TRACK_VALGRIND || PA_TRACK_ASAN
  (void)p; (void)size;
