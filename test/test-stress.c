...
//
// argument defaults
#if defined(PA_TSAN)          // with thread-sanitizer reduce the threads to test within the azure pipeline limits
static int THREADS = 8;
static int SCALE   = 25;
static int ITER    = 400;
#elif defined(PA_UBSAN)       // with undefined behavious sanitizer reduce parameters to stay within the azure pipeline limits
static int THREADS = 8;
static int SCALE   = 25;
static int ITER    = 20;
#elif defined(PA_GUARDED)     // with debug guard pages reduce parameters to stay within the azure pipeline limits
static int THREADS = 8;
static int SCALE   = 10;
static int ITER    = 10;
#else
...
#define custom_calloc(n,s)    pa_calloc(n,s)
#define custom_realloc(p,s)   pa_realloc(p,s)
#define custom_free(p)        pa_free(p)
...
#ifdef HEAP_WALK
static bool visit_blocks(const pa_heap_t* heap, const pa_heap_area_t* area, void* block, size_t block_size, void* arg) {
  (void)(heap); (void)(area);
...
static void stress(intptr_t tid) {
  ...
  // walk the heap
  size_t total = 0;
  pa_heap_visit_blocks(pa_heap_get_default(), true, visit_blocks, &total);
  #endif
...
static void test_stress(void) {
  ...
  for (int n = 0; n < ITER; n++) {
    ...
    // pa_option_set_enabled(pa_option_disallow_arena_alloc, (n%2)==1);
    #endif
    #ifdef HEAP_WALK
    size_t total = 0;
    pa_abandoned_visit_blocks(pa_subproc_main(), -1, true, visit_blocks, &total);
    #endif
    ...
    #ifndef NDEBUG
    //pa_collect(false);
    //pa_debug_show_arenas(true);
    #endif
    #if !defined(NDEBUG) || defined(PA_TSAN)
...
#if defined(USE_STD_MALLOC) && defined(PA_LINK_VERSION)
#ifdef __cplusplus
extern "C"
#endif
int pa_version(void);
...
int main(int argc, char** argv) {
  #ifdef PA_LINK_VERSION
    pa_version();
  #endif
  #ifdef HEAP_WALK
    pa_option_enable(pa_option_visit_abandoned);
  #endif
  #if !defined(NDEBUG) && !defined(USE_STD_MALLOC)
    pa_option_set(pa_option_arena_reserve, 32 * 1024 /* in kib = 32MiB */);
  #endif
  ...
  //pa_reserve_os_memory(1024*1024*1024ULL, false, true);
  //int res = pa_reserve_huge_os_pages(4,1);
  ...
  //pa_reserve_os_memory(512ULL << 20, true, true);
  ...
  pa_collect(true);
  char* json = pa_stats_get_json(0, NULL);
  if (json != NULL) {
    fputs(json,stderr);
    pa_free(json);
  }
