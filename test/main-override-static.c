...
int main() {
  ...
  pa_heap_t* h = pa_heap_new();
  pa_heap_set_default(h);
...
static void test_align() {
  void* p = pa_malloc_aligned(256, 256);
...
static void block_overflow1() {
  uint8_t* p = (uint8_t*)pa_malloc(17);
  p[18] = 0;
  free(p);
}

static void block_overflow2() {
  uint8_t* p = (uint8_t*)pa_malloc(16);
  p[17] = 0;
  free(p);
}
...
static void double_free1() {
  ...
  p[0] = pa_malloc(622616);
  p[1] = pa_malloc(655362);
  p[2] = pa_malloc(786432);
  pa_free(p[2]);
  // [VULN] Double free
  pa_free(p[2]);
  p[3] = pa_malloc(786456);
  // [BUG] Found overlap
...
static void test_process_info(void) {
  ...
  pa_process_info(&elapsed, &user_msecs, &system_msecs, &current_rss, &peak_rss, &current_commit, &peak_commit, &page_faults);
...
#define MiB (KiB*KiB)
#define GiB (MiB*KiB)
  pa_reserve_os_memory(4*GiB, false, true);
  void* p1 = malloc(100);
...
static void negative_stat(void) {
  int* p = pa_malloc(60000);
  pa_stats_print_out(NULL, NULL);
  *p = 100;
  pa_free(p);
  pa_stats_print_out(NULL, NULL);
}
...
static bool test_visit(const pa_heap_t* heap, const pa_heap_area_t* area, void* block, size_t block_size, void* arg) {
  if (block == NULL) {
    printf("visiting an area with blocks of size %zu (including padding)\n", area->full_block_size);
  }
  else {
    printf("  block of size %zu (allocated size is %zu)\n", block_size, pa_usable_size(block));
  }
  return true;
}
...
static void test_heap_walk(void) {
  ...
  pa_heap_malloc(heap, 16*2097152);
  pa_heap_malloc(heap, 2067152);
  pa_heap_malloc(heap, 2097160);
  pa_heap_malloc(heap, 24576);
  pa_heap_visit_blocks(heap, true, &test_visit, NULL);
}

static void test_heap_arena(void) {
  pa_arena_id_t arena_id;
  int err = pa_reserve_os_memory_ex(100 * 1024 * 1024, false /* commit */, false /* allow large */, true /* exclusive */, &arena_id);
  if (err) abort();
  pa_heap_t* heap = pa_heap_new_in_arena(arena_id);
  for (int i = 0; i < 500000; i++) {
    void* p = pa_heap_malloc(heap, 1024);
    if (p == NULL) {
...
static void test_manage_os_memory(void) {
  ...
  pa_arena_id_t arena_id;
  pa_manage_os_memory_ex(ptr, size, true /* committed */, true /* pinned */, false /* is zero */, -1 /* numa node */, true /* exclusive */, &arena_id);
  pa_heap_t* cuda_heap = pa_heap_new_in_arena(arena_id);    // you can do this in any thread

  // now allocate only in the cuda arena
  void* p1 = pa_heap_malloc(cuda_heap, 8);
  int* p2 = pa_heap_malloc_tp(cuda_heap, int);
  *p2 = 42;
  
  // and maybe set the cuda heap as the default heap? (but careful as now `malloc` will allocate in the cuda heap as well)
  {
    pa_heap_t* prev_default_heap = pa_heap_set_default(cuda_heap);
    void* p3 = pa_malloc(8);  // allocate in the cuda heap 
    pa_free(p3);
  }
  pa_free(p1);
  pa_free(p2);
...
static void test_large_pages(void) {
  ...
  size_t page_size;
  uint8_t* p = (uint8_t*)_pa_os_alloc_huge_os_pages(1, -1, 30000, &pages_reserved, &page_size, &memid);
  const size_t req_size = pages_reserved * page_size;
  #else
  const size_t req_size = 64*PA_MiB;
  uint8_t* p = (uint8_t*)_pa_os_alloc(req_size,&memid,NULL);
  #endif

  p[0] = 1;

  //_pa_os_protect(p, _pa_os_page_size());
  //_pa_os_unprotect(p, _pa_os_page_size());
  //_pa_os_decommit(p, _pa_os_page_size(), NULL);
  if (madvise(p, req_size, MADV_HUGEPAGE) == 0) {
    printf("advised huge pages\n");
    _pa_os_decommit(p, _pa_os_page_size(), NULL);
  };
  _pa_os_free(p, req_size, memid, NULL);
}
...
#define PA_INTPTR_SIZE 8
#define PA_LARGE_WSIZE_MAX (4*1024*1024 / PA_INTPTR_SIZE)

#define PA_BIN_HUGE 100
//#define PA_ALIGN2W

// Bit scan reverse: return the index of the highest bit.
static inline uint8_t pa_bsr32(uint32_t x);
...
// Bit scan reverse: return the index of the highest bit.
uint8_t _pa_bsr(uintptr_t x) {
  if (x == 0) return 0;
  #if PA_INTPTR_SIZE==8
  uint32_t hi = (x >> 32);
  return (hi == 0 ? pa_bsr32((uint32_t)x) : 32 + pa_bsr32(hi));
...
static inline size_t pa_bin(size_t wsize) {
  ...
  */
  ...
  if (wsize <= 8) {
...
#endif
  else if (wsize > PA_LARGE_WSIZE_MAX) {
    return PA_BIN_HUGE;
  }
  else {
#if defined(PA_ALIGN4W)
    ...
    const size_t b = _pa_bsr(wsize);  // note: wsize != 0
    // and use the top 3 bits to determine the bin (~12.5% worst internal fragmentation).
    // - adjust with 3 because we use do not round the first 8 sizes
    //   which each get an exact bin
    const size_t bin = ((b << 2) + ((wsize >> (b - 2)) & 0x03)) - 3;
    assert(bin > 0 && bin < PA_BIN_HUGE);
    return bin;
  }
}


static inline uint8_t _pa_bin4(size_t size) {
  size_t wsize = _pa_wsize_from_size(size);
  uint8_t bin;
  if (wsize <= 1) {
    bin = 1;
    ...
    uint8_t b = pa_bsr32((uint32_t)wsize);
    bin = ((b << 1) + (uint8_t)((wsize >> (b - 1)) & 0x01)) + 3;
  }
  return bin;
}

static size_t _pa_binx4(size_t wsize) {
  size_t bin;
  if (wsize <= 1) {
    bin = 1;
  }
...
static size_t _pa_binx8(size_t bsize) {
  if (bsize<=1) return bsize;
  uint8_t b = pa_bsr32((uint32_t)bsize);
  if (b <= 2) return bsize;
...
static inline size_t pa_binx(size_t wsize) {
  uint8_t bin;
  if (wsize <= 1) {
    bin = 1;
  }
    ...
    wsize--;
    // find the highest bit
    uint8_t b = (uint8_t)pa_bsr32((uint32_t)wsize);  // note: wsize != 0
    // and use the top 3 bits to determine the bin (~12.5% worst internal fragmentation).
    // - adjust with 3 because we use do not round the first 8 sizes
    //   which each get an exact bin
