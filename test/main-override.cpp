...
int main() {
  pa_stats_reset();  // ignore earlier allocations
  // various_tests();
  // test_mixed1();
  // test_dep();
...
static void various_tests() {
  atexit(free_p);
  void* p1 = malloc(78);
  void* p2 = pa_malloc_aligned(24, 16);
  free(p1);
  p1 = malloc(8);
  char* s = pa_strdup("hello\n");
...
#if PA_HAS_HEAP_STL_ALLOCATOR
static bool test_stl_allocator3() {
  std::vector<int, pa_heap_stl_allocator<int> > vec;
  vec.push_back(1);
  vec.pop_back();
  return vec.size() == 0;
}
...
static bool test_stl_allocator5() {
  std::vector<int, pa_heap_destroy_stl_allocator<int> > vec;
  vec.push_back(1);
  vec.pop_back();
  return vec.size() == 0;
}
...
{
  cptr = pa_calloc_tp(char,22085632);
  return NULL;
}
...
static void heap_no_delete_worker() {
  ...
  void* q = pa_heap_malloc(heap, 1024); (void)(q);
  // pa_heap_delete(heap); // uncomment to prevent assertion
}
...
static void t1main() {
  pa_heap_t* heap = pa_heap_new();
  global_p = pa_heap_malloc(heap, 1024);
  pa_heap_delete(heap);
}
...
static void heap_thread_free_large() {
  for (int i = 0; i < 100; i++) {
    shared_p = pa_malloc_aligned(2 * 1024 * 1024 + 1, 8);
...
static void heap_thread_free_huge() {
  for (int i = 0; i < 100; i++) {
    shared_p = pa_malloc(1024 * 1024 * 1024);
...
static void test_perf_local_alloc()
{
  ...
  {
    int len = LEN[rng() % 4];
    int* p = (int*)pa_zalloc_aligned(len * sizeof(int), alignof(int));
    p[0] = 1;
    sum += p[rng() % len];
    free(p);
  }
