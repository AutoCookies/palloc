...
2. Link your program explicitly with the `palloc.dll.lib` export library for
   ...
   to insert some call to the palloc API in the `main` function, like `pa_version()`
   (or use the `/include:pa_version` switch on the linker, or
   similarly, `#pragma comment(linker, "/include:pa_version")` in some source file). 
...
4. Ensure the `palloc.dll` comes as early as possible in the import
   ...
   You can use `painject -l <exe>` to check this if needed.
...
The `painject` program can also do this from the command line
Use `painject --help` for options:
...
examples:
  > painject --list myprogram.exe
  > painject --force --inplace myprogram.exe
```  

For x86 32-bit binaries, use `painject32`, and for arm64 binaries use `painject-arm64`.
