...
#include "internal.h"             // pa_decl_hidden
...
typedef struct pa_os_mem_config_s {
...
// which will later be committed explicitly using `_pa_prim_commit`.
// `is_zero` is set to true if the memory was zero initialized (as on most OS's)
// The `hint_addr` address is either `NULL` or a preferred allocation address but can be ignored.
// pre: !commit => !allow_large
//      try_alignment >= _pa_os_page_size() and a power of 2
...
// may have been reset (`_pa_prim_reset`) or decommitted (`_pa_prim_decommit`) where `needs_recommit` was false.
...
typedef struct pa_process_info_s {
  pa_msecs_t  elapsed;
  pa_msecs_t  utime;
  pa_msecs_t  stime;
  size_t      current_rss;
...
// Called on the first thread start, and should ensure `_pa_thread_done` is called on thread termination.
...
// We need fast access to both a unique thread id (in `free.c:pa_free`) and
// to a thread-local heap pointer (in `alloc.c:pa_malloc`).
...
#define PA_HAS_TLS_SLOT    1

static inline void* pa_prim_tls_slot(size_t slot) pa_attr_noexcept {
  void* res;
  ...
  #elif defined(__x86_64__) && (PA_INTPTR_SIZE==4)
    __asm__("movl %%fs:%1, %0" : "=r" (res) : "m" (*((void**)ofs)) : );  // x32 ABI
  #elif defined(__x86_64__)
    __asm__("movq %%fs:%1, %0" : "=r" (res) : "m" (*((void**)ofs)) : );  // x86_64 Linux, BSD uses FS
  #elif defined(__arm__)
    void** tcb; PA_UNUSED(ofs);
...
static inline void pa_prim_tls_slot_set(size_t slot, void* value) pa_attr_noexcept {
  const size_t ofs = (slot*sizeof(void*));
  #if defined(__i386__)
...
#elif _WIN32 && PA_WIN_USE_FIXED_TLS && !defined(PA_WIN_USE_FLS)
...
// and store the offset `_pa_win_tls_offset`.
#define PA_HAS_TLS_SLOT  1              // 2 = we can reliably initialize the slot (saving a test on each malloc)

extern pa_decl_hidden size_t _pa_win_tls_offset;

#if PA_WIN_USE_FIXED_TLS > 1
#define PA_TLS_SLOT     (PA_WIN_USE_FIXED_TLS)
#elif PA_SIZE_SIZE == 4
#define PA_TLS_SLOT     (0x0E10 + _pa_win_tls_offset)  // User TLS slots <https://en.wikipedia.org/wiki/Win32_Thread_Information_Block>
#else
#define PA_TLS_SLOT     (0x1480 + _pa_win_tls_offset)  // User TLS slots <https://en.wikipedia.org/wiki/Win32_Thread_Information_Block>
#endif
...
// fast path of `_pa_free` and we specialize for various platforms as
// inlined definitions. Regular code should call `init.c:_pa_thread_id()`.
// We only require _pa_prim_thread_id() to return a unique id
...
// For now, we only enable this for specific platforms.
    ...
    #define PA_USE_BUILTIN_THREAD_POINTER  1
  #endif
#endif
...
extern pa_decl_hidden pa_decl_thread pa_heap_t* _pa_heap_default;  // default heap to allocate from
extern pa_decl_hidden bool _pa_process_is_initialized;             // has pa_process_init been called?

static inline pa_threadid_t _pa_prim_thread_id(void) pa_attr_noexcept;

// Get a unique id for the current thread.
#if defined(PA_PRIM_THREAD_ID)

static inline pa_threadid_t _pa_prim_thread_id(void) pa_attr_noexcept {
  return PA_PRIM_THREAD_ID();  // used for example by CPython for a free threaded build (see python/cpython#115488)
}
...
#elif PA_USE_BUILTIN_THREAD_POINTER
...
#elif PA_HAS_TLS_SLOT

static inline pa_threadid_t _pa_prim_thread_id(void) pa_attr_noexcept {
  #if defined(__BIONIC__)
    // issue #384, #495: on the Bionic libc (Android), slot 1 is the thread id
    // see: https://github.com/aosp-mirror/platform_bionic/blob/c44b1d0676ded732df4b3b21c5f798eacae93228/libc/platform/bionic/tls_defines.h#L86
    return (uintptr_t)pa_prim_tls_slot(1);
  #else
    // in all our other targets, slot 0 is the thread id
    // glibc: https://sourceware.org/git/?p=glibc.git;a=blob_plain;f=sysdeps/x86_64/nptl/tls.h
    // apple: https://github.com/apple/darwin-xnu/blob/main/libsyscall/os/tsd.h#L36
    return (uintptr_t)pa_prim_tls_slot(0);
  #endif
}
...
static inline pa_threadid_t _pa_prim_thread_id(void) pa_attr_noexcept {
  return (uintptr_t)&_pa_heap_default;
}
...
/* ----------------------------------------------------------------------------------------
Get the thread local default heap: `_pa_prim_get_default_heap()`

This is inlined here as it is on the fast path for allocation functions.

On most platforms (Windows, Linux, FreeBSD, NetBSD, etc), this just returns a
__thread local variable (`_pa_heap_default`). With the initial-exec TLS model this ensures
that the storage will always be available (allocated on the thread stacks).

On some platforms though we cannot use that when overriding `malloc` since the underlying
TLS implementation (or the loader) will call itself `malloc` on a first access and recurse.
We try to circumvent this in an efficient way:
- macOSX : we use an unused TLS slot from the OS allocated slots (PA_TLS_SLOT). On OSX, the
           loader itself calls `malloc` even before the modules are initialized.
- OpenBSD: we use an unused slot from the pthread block (PA_TLS_PTHREAD_SLOT_OFS).
- DragonFly: defaults are working but seem slow compared to freeBSD (see PR #323)
------------------------------------------------------------------------------------------- */

static inline pa_heap_t* pa_prim_get_default_heap(void);

#if defined(PA_MALLOC_OVERRIDE)
#if defined(__APPLE__) // macOS
  #define PA_TLS_SLOT               89  // seems unused?
  // other possible unused ones are 9, 29, __PTK_FRAMEWORK_JAVASCRIPTCORE_KEY4 (94), __PTK_FRAMEWORK_GC_KEY9 (112) and __PTK_FRAMEWORK_OLDGC_KEY9 (89)
  // see <https://github.com/rweichler/substrate/blob/master/include/pthread_machdep.h>
#elif defined(__OpenBSD__)
  // use end bytes of a name; goes wrong if anyone uses names > 23 characters (ptrhread specifies 16)
  // see <https://github.com/openbsd/src/blob/master/lib/libc/include/thread_private.h#L371>
  #define PA_TLS_PTHREAD_SLOT_OFS   (6*sizeof(int) + 4*sizeof(void*) + 24)
  // #elif defined(__DragonFly__)
  // #warning "palloc is not working correctly on DragonFly yet."
  // #define PA_TLS_PTHREAD_SLOT_OFS   (4 + 1*sizeof(void*))  // offset `uniqueid` (also used by gdb?) <https://github.com/DragonFlyBSD/DragonFlyBSD/blob/master/lib/libthread_xu/thread/thr_private.h#L458>
#elif defined(__ANDROID__)
  // See issue #381
  #define PA_TLS_PTHREAD
#endif
#endif


#if PA_TLS_SLOT
# if !defined(PA_HAS_TLS_SLOT)
#  error "trying to use a TLS slot for the default heap, but the pa_prim_tls_slot primitives are not defined"
# endif

static inline pa_heap_t* pa_prim_get_default_heap(void) {
  pa_heap_t* heap = (pa_heap_t*)pa_prim_tls_slot(PA_TLS_SLOT);
  #if PA_HAS_TLS_SLOT == 1   // check if the TLS slot is initialized
  if pa_unlikely(heap == NULL) {
    #ifdef __GNUC__
    __asm(""); // prevent conditional load of the address of _pa_heap_empty
    #endif
    heap = (pa_heap_t*)&_pa_heap_empty;
  }
  #endif
  return heap;
}
...
static inline pa_heap_t** pa_prim_tls_pthread_heap_slot(void) {
  pthread_t self = pthread_self();
  #if defined(__DragonFly__)
  if (self==NULL) return NULL;
  #endif
  return (pa_heap_t**)((uint8_t*)self + PA_TLS_PTHREAD_SLOT_OFS);
}

static inline pa_heap_t* pa_prim_get_default_heap(void) {
  pa_heap_t** pheap = pa_prim_tls_pthread_heap_slot();
  if pa_unlikely(pheap == NULL) return _pa_heap_main_get();
  pa_heap_t* heap = *pheap;
  if pa_unlikely(heap == NULL) return (pa_heap_t*)&_pa_heap_empty;
  return heap;
}

#elif defined(PA_TLS_PTHREAD)

extern pa_decl_hidden pthread_key_t _pa_heap_default_key;
static inline pa_heap_t* pa_prim_get_default_heap(void) {
  pa_heap_t* heap = (pa_unlikely(_pa_heap_default_key == (pthread_key_t)(-1)) ? _pa_heap_main_get() : (pa_heap_t*)pthread_getspecific(_pa_heap_default_key));
  return (pa_unlikely(heap == NULL) ? (pa_heap_t*)&_pa_heap_empty : heap);
}

#else // default using a thread local variable; used on most platforms.

static inline pa_heap_t* pa_prim_get_default_heap(void) {
  #if defined(PA_TLS_RECURSE_GUARD)
  if (pa_unlikely(!_pa_process_is_initialized)) return _pa_heap_main_get();
  #endif
  return _pa_heap_default;
}

#endif  // pa_prim_get_default_heap()
